...
def escape(carpark):
    ...
    for floor in carpark:
        if 2 in floor:
            car_loc = floor.index(2)
            car_floor = carpark.index(floor)
            break
    
    for floor in carpark[car_floor:-1]:
        stair_loc = floor.index(1)

        if stair_loc > car_loc:
            result.append("R%s" % (stair_loc - car_loc))
            car_lock = stair_loc

        elif stair_loc < car_loc:
            result.append("L%s" % (car_loc - stair_loc))
        
        if result[-1].startswith("D"):
            result[-1] = "D%s" % (int(result[-1][1]) + 1)
        else:
            result.append("D1")
        car_loc = stair_loc
    
    exit_loc = len(carpark[0]) - 1
    if car_loc != exit_loc:
        result.append("R%s" % (exit_loc - car_loc))
